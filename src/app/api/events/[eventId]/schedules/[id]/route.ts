...
export async function PUT(
  ...
  try {
    ...
    if (!session || !checkPermission(session.role, 'schedules', 'update')) {
    ...
    await logAction(request, session.userId, 'update', 'Schedule', id, {
      title: itemData.title,
    });

    return NextResponse.json({ success: true, data: itemData, message: 'Jadwal diperbarui' });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    if (force) {
      ...
      await logAction(request, session.userId, 'hard_delete', 'Schedule', id);
    } else {
      const softDeleted = await Schedule.findOneAndUpdate(
        { _id: id, deletedAt: null },
        { deletedAt: new Date(), deletedBy: session.userId },
        { new: true }
      );
      if (!softDeleted) {
        return NextResponse.json({ success: false, message: 'Jadwal tidak ditemukan atau sudah dihapus' }, { status: 404 });
      }
      await logAction(request, session.userId, 'delete', 'Schedule', id);
    }

    return NextResponse.json({
      success: true,
      message: force ? 'Jadwal dihapus permanen!' : 'Jadwal berhasil dihapus (soft delete)!',
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Server error';
    return NextResponse.json({ success: false, error: message }, { status: 500 });
